function [sol, incr, incr2, iter, ACOC] = Problema9Jarratt(f, fy, fdy, N, tol, maxiter)
    ...
    while incr(iter + 1) + incr2(iter + 1) > tol && iter < maxiter
        ...
        mDiag2 = zeros(N, 1); lDiag2 = zeros(N - 1, 1); uDiag2 = zeros(N - 1, 1);
        
        zk(1) = (y(2) - h * y(1)) / h;
        mDiag2(1) = ((2 * h - 2) + (h^2) * (fy(xk(1), y(1), zk(1)) - fdy(xk(1), y(1), zk(1))));
        uDiag2(1) = (2 + h * fdy(xk(1), y(1), zk(1)));

        for i = 2 : (N - 1)
            zk(i) = (y(i + 1) - y(i - 1)) / (2 * h);
            mDiag2(i) = (-2 + h^2 * fy(xk(i), y(i), zk(i)));
            uDiag2(i) =  (1 + (h/2) * fdy(xk(i), y(i), zk(i)));
            lDiag2(i - 1) =  (1 - (h/2) * fdy(xk(i), y(i), zk(i)));
        end

        zk(N) = (y(N) - 2) / 2;
        mDiag2(N) =  ((h - 2) + h^2 * (fy(xk(N), y(N), zk(N)) + (1/2) * fdy(xk(N), y(N), zk(N))));
        lDiag2(N - 1) =  (2);

        valdF = diag(mDiag) + diag(uDiag, 1) + diag(lDiag, -1);
        valdF2 = 3 * (diag(mDiag2) + diag(uDiag2, 1) + diag(lDiag2, -1));

        c = (valdF2 + valdF) * z;

        z = (1/2) * ((valdF2 - valdF) \ c);
