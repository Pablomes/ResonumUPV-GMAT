function [yk, xk] = Problema16b(F, a, b, alpha, beta, N, t0, t1, tol, maxiter)
...
[Yk, ~] = MetodoRK4Sistemas(F, a, b, [alpha; t0], N);      y0 = Yk(1, end); dy0 = Yk(2, end);
[Yk, ~] = MetodoRK4Sistemas(F, a, b, [alpha; t1], N);      y1 = Yk(1, end); dy1 = Yk(2, end);

while and(S > tol, iter < maxiter)
    t2 = t1 - (dy1 - 0.083*(y1 - beta)) * (t1 - t0) / (dy1 - 0.083*y1 - (dy0 - 0.083*y0));
    [Yk, ~] = MetodoRK4Sistemas(F, a, b, [alpha; t2], N);
    y0 = y1; dy0 = dy1; y1 = Yk(1, end); dy1 = Yk(2, end); S = abs(dy1 - 0.083*(y1 - beta));
