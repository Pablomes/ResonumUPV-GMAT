function [yk, xk, T] = Ej1(F, tol, maxiter)
...
N = 10; a = 2; b = 3; t0 = 0; t1 = 1;
h = (b-a) / N; xk = a:h:b; iter = 0; S = tol + 1;
alpha = exp(-2); beta = exp(-3); T = [t0, t1];

[Yk, ~] = MetodoRK4Sistemas(F, a, b, [t0; alpha - t0], N);      y0 = Yk(1, end); dy0 = Yk(2, end);
[Yk, ~] = MetodoRK4Sistemas(F, a, b, [t1; alpha - t1], N);      y1 = Yk(1, end); dy1 = Yk(2, end);

while and(S > tol, iter < maxiter)
    t2 = t1 - (y1 + dy1 - beta) * (t1 - t0) / (y1 + dy1 - y0 - dy0);
    T = [T, t2];
    [Yk, ~] = MetodoRK4Sistemas(F, a, b, [t2; alpha - t2], N);
