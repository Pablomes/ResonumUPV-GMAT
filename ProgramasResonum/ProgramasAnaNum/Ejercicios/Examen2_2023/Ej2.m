function [yk, xk] = Ej2(F,Fy1,Fy2, t0,tol,maxiter)
...
a = 2; b = 3; alpha = exp(-2); beta = exp(-3); N = 10;

h = (b - a) / N; xk = a:h:b; iter = 0; S = tol + 1;
FN = @(x, Y) [Y(2); F(x, Y(1), Y(2)); Y(4); Fy1(x, Y(1), Y(2)) * Y(3) + Fy2(x, Y(1), Y(2)) * Y(4)];

[Yk, ~] = MetodoRK4Sistemas(FN, a, b, [t0;alpha - t0; 1; -1], N); 
y1b = Yk(1, end); dy1b = Yk(2, end); y3b = Yk(3, end); dy3b = Yk(4, end);

while and(S > tol, iter < maxiter)

    t1 = t0 - (y1b + dy1b - beta) / (y3b + dy3b);
    [Yk, ~] = MetodoRK4Sistemas(FN, a, b, [t1; alpha - t1; 1; -1], N); 
    y1b = Yk(1, end); dy1b = Yk(2, end); y3b = Yk(3, end); dy3b = Yk(4, end); S = abs(y1b + dy1b - beta);
    t0 = t1; iter = iter + 1;
end
